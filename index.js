...
export default class AnimateNumber extends Component {
  ...
  constructor(props: any) {
    ...
    var {initial, formatter} = this.props
    this.state = {
      value: initial || 0,
      displayValue: (initial && formatter) ? formatter(initial) : 0
    };
    this.dirty = false;
    this.startFrom = 0;
    this.endWith = 0;
  ...
  getTimingFunction(interval: number, progress: number) {
    ...
    } else if (typeof this.props.timing === 'function') return this.props.timing(interval, progress);
